...
ENV_SERVICE_USER = os.environ.get(ENV_NAME_SERVICE_USER, None)
ENV_SERVICE_PASSWORD = os.environ.get(ENV_NAME_SERVICE_PASSWORD, None)
...
if ENV_NAME_SERVICE_SSL_ENABLED in os.environ \
    ...
    call(
        f"sed -i 's@#ssl_certificate_key@ssl_certificate_key {ENV_SSL_RESOURCES_PATH}/cert.key;@g' {NGINX_FILE}",
        shell=True,
    )
    call(
        f"sed -i 's@#ssl_certificate@ssl_certificate {ENV_SSL_RESOURCES_PATH}/cert.crt;@g' {NGINX_FILE}",
        shell=True,
    )
    ...
    call(f"{ENV_RESOURCES_PATH}/scripts/setup-certs.sh", shell=True)
...
if ENV_SERVICE_USER and ENV_SERVICE_PASSWORD:
    ...
    call(f"sed -i 's/#auth_basic /auth_basic /g' {NGINX_FILE}", shell=True)
    call(
        f"sed -i 's/#auth_basic_user_file/auth_basic_user_file/g' {NGINX_FILE}",
        shell=True,
    )

    # create basic auth user
    call(
        f"echo '{ENV_SERVICE_PASSWORD}' | htpasswd -b -i -c /etc/nginx/.htpasswd '{ENV_SERVICE_USER}'",
        shell=True,
    )
###
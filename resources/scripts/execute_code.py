...
if not code_path or not os.path.exists(code_path):
    log.info(f"No code artifacts could be found for {EXECUTE_CODE}")
    sys.exit(1)
...
if os.path.isdir(code_path):
    ...
    if not args.code_only:
        log.info(f"Searching requirements at path {main_script}")
        ...
        if os.path.isfile(conda_env_path):
            ...
            log.info(f"Installing conda environment from {conda_env_path}")
            if (
                call(
                    f"conda env create -n {conda_env_name} -f {conda_env_path}"
                )
                == 0
            ):
                # Set pip and python runtime to the conda environment
                pip_runtime = f"{CONDA_ROOT}/envs/{conda_env_name}/bin/pip"
                python_runtime = f"{CONDA_ROOT}/envs/{conda_env_name}/bin/python"
                # put conda env into the bash runtime
                bash_runtime = f"PATH={CONDA_ROOT}/envs/{conda_env_name}/bin/:$PATH /bin/bash"
            else:
                log.info(f"Failed to install conda env from {conda_env_path}")
        ...
        if os.path.isfile(setup_path):
            log.info(f"Running setup from {setup_path}")
            if call(f"{bash_runtime} {setup_path}") != 0:
                log.info(f"Failed to run setup.sh from {setup_path}")

        # Check for requirements.txt file
        requirements_path = os.path.join(code_path, "requirements.txt")
        if os.path.isfile(requirements_path):
            log.info(f"Installing requirements from {requirements_path}")
            if (
                call(
                    f"{pip_runtime} install --no-cache-dir -r {requirements_path}"
                )
                != 0
            ):
                log.info(f"Failed to install requirements.txt from {requirements_path}")

    if args.requirements_only:
        log.info("Finished installing requirements. Code execution is deactivated.")
        sys.exit(0)

    log.info(f"Executing python code at path {main_script}")
    # Run code: if it is a folder, it needs a main module (e.g. __main__.py)
    exit_code = call(f'{python_runtime} "{main_script}"')
    if exit_code > 0:
        log.info(f"Execution failed with exit code: {str(exit_code)}")
        if os.path.isdir(main_script):
            log.info(
                f"Please make sure that there is a main module (e.g. __main__.py) at this path: {main_script}"
            )
    else:
